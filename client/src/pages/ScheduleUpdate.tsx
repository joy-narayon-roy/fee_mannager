...
import { Profile, Schedule } from "../models";
...
interface Props {
    exist_schedule: Schedule
    profile: Profile
}
function ScheduleCreate({ exist_schedule, profile }: Props) {
    const nav = useNavigate()
    const initialSchedule: ScheduleType = {
        days: exist_schedule?.days || [],
        is_active: exist_schedule.is_active || false,
        name: exist_schedule.name || '',
        note: exist_schedule.note || "",
    }
    ...
    const handleSubmit: React.FormEventHandler = async (ev) => {
        ev.preventDefault()
        try {
            const { data } = await api.patch('/schedule/' + exist_schedule.id, state)
...
function ScheduleUpdate() {
    const { id = '' } = useParams()
    const { profile, loading } = useMainContext()
    const exist_schedule = profile.getScheduleByID(id)
    return (
        <>
            {loading && <>Loading...</>}
            {exist_schedule ? <ScheduleCreate exist_schedule={exist_schedule} profile={profile} /> : <h1 className="text-center text-3xl mt-10">Schdule not found!</h1>}
        </>
    )
}
